class ProductList {
    constructor(productsUrl, renderContainer, cart) {
        ...
        fetch(productsUrl).then(result=>result.json()).then(products=>{
            ...
            document.querySelector('.filter').addEventListener('keydown', ()=>this.renderProducts(renderContainer, products));
        }
        );
    }
    getProductById(id) {
    ...
    renderProducts(container, products) {
        ...
        products.filter(product=>product.title.toLowerCase().includes(document.querySelector('.filter').value.toLowerCase())).forEach(product=>{
